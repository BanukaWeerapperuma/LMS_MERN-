...
const SearchBar = ({data}) => {
  ...
  const [input ,setInut] = React.useState(data ? data : '')
  ...
  return (
    <form  onSubmit={onSearchHandler} className='max-w-xl w-full md:h-14 h-12 flex items-center bg-white border border-gray-500/20 rounded'>
      <img src={assets.search_icon} alt="search-icon"
        className='md:w-auto w10 px-3' />
      <input onChange={(e) => setInut(e.target.value)} value={input}
      type="text" placeholder="Search for a courses"
